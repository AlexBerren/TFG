...
public class OtherFollowersListFragment extends Fragment {
    ...
    public OtherFollowersListFragment() {

    }

    public static OtherFollowersListFragment newInstance(String param1, String param2) {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_other_followers_list, container, false);
        ...
        recyclerSeguidores = view.findViewById(R.id.recyclerJugadores);
        botonAtras = view.findViewById(R.id.chatTooolbarIcon);
        recyclerSeguidores.setLayoutManager(new LinearLayoutManager(getActivity()));
        ...
        if (bundle != null) {
            idDocumento = bundle.getString("ID_DOCUMENTO");
            fragmentProvider = bundle.getString("Fragment_Provider");

            // Hacer algo con el valor del ID del documento recuperado
        }

        DocumentReference usuariosRef = db.collection("users").document(idDocumento).collection("followers").document("usernames");
        ...
        DocumentReference usuarioNombre = db.collection("users").document(idDocumento);
        usuarioNombre.get().addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                DocumentSnapshot document = task.getResult();
                if (document.exists()) {
                    String username = document.getString("username");
                    toolbarTitulo.setText("Seguidores de @" + username);
                }
            }
        });
